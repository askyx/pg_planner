...
* required 是外部或者当前ctx要求的，是优化器的输入属性
  * 外部输入属性，从 root 开始检测 child 有没有满足的属性，没有则推导出要求的属性，然后构建对应的 prop ctx
* derived 是由优化器推导出的属性，是依赖于子节点的属性，用于检查是否满足required，不满足则使用enforcer添加属性
  * enforcer 添加属性之后，节点变成树的型结构的片段，例如
    * sort 节点
    * exchange 节点
    * memorize 节点
  * 此时添加到那里
